...
Rectangle {
    ...
    function updateLine(): void {
        ...
        // Add one past the top-right corner to avoid the line being drawn on the right side
        newPoints.push(Qt.point(root.width + 1, 0));

        // Ditto but for the bottom-right corner
        newPoints.push(Qt.point(root.width + 1, root.height + 1));

        // And one last point for the bottom left corner
    ...
    component ControlCircle: Rectangle {
        ...
        readonly property bool selected: root.activeFocus && root.selectedControlPoint === index
        ...
        border {
            width: selected? 2 : 1
            color: selected ? Kirigami.Theme.highlightColor : Kirigami.Theme.disabledTextColor
        }
    ...
    Repeater {
        ...
        delegate: ControlCircle {
            ...
            visible: !isFirstPoint(index) && !isLastPoint(index)
